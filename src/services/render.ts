...
<head>
  ...
  <title>${title}</title>
  <meta name="description" content="TypeScript command center for booking disruptions, recovery queues, guest communication posture, and launch-safe hospitality operations. Operator surface in the Kinetic Gain Suite." />
  ...
  <link rel="canonical" href="https://bookings.kineticgain.com/" />
  <link rel="icon" type="image/svg+xml" href="${KG_FAVICON_DATA_URI}" />
  <meta property="og:type" content="website" />
  <meta property="og:site_name" content="Kinetic Gain" />
  <meta property="og:title" content="${title}" />
  <meta property="og:url" content="https://bookings.kineticgain.com/" />
  <meta property="og:description" content="TypeScript command center for booking disruptions, recovery queues, guest communication posture, and launch-safe hospitality operations. Operator surface in the Kinetic Gain Suite." />
  <meta name="twitter:card" content="summary" />
  <meta name="twitter:title" content="${title}" />
  <meta name="twitter:description" content="TypeScript command center for booking disruptions, recovery queues, guest communication posture, and launch-safe hospitality operations. Operator surface in the Kinetic Gain Suite." />
  <script type="application/ld+json">{"@context":"https://schema.org","@graph":[{"@type":"ProfessionalService","@id":"https://bookings.kineticgain.com/#service","name":"Kinetic Gain — TypeScript command center for booking disruptions, recovery queues, guest communication posture, and launch-safe hospitality operations","url":"https://bookings.kineticgain.com/","areaServed":"United States","description":"TypeScript command center for booking disruptions, recovery queues, guest communication posture, and launch-safe hospitality operations. Operator surface in the Kinetic Gain Suite."},{"@type":"WebSite","@id":"https://bookings.kineticgain.com/#website","url":"https://bookings.kineticgain.com/","name":"TypeScript command center for booking disruptions, recovery queues, guest communication posture, and launch-safe hospitality operations","publisher":{"@type":"Organization","name":"Kinetic Gain LLC"}}]}</script>
  <style>${KG_STYLE01_CSS}${KG_COMPAT_CSS}${MENU_SURFACE_CSS}</style>
</head>
<body>
  <header>
    <div class="wrap nav">
      <a class="kg-logo" href="/" aria-label="Kinetic Gain — TypeScript command center for booking disruptions, recovery queues, guest communication posture, and launch-safe hospitality operations">
        ${KG_MARK_SVG}
        <span class="kg-word">Kinetic Gain</span>
      </a>
      <nav class="nav-links" id="primaryNav">
        <a href="/booking-lane">Booking Lane</a>
        <a href="/recovery-risks">Recovery Risks</a>
        <a href="/guest-communication">Guest Communication</a>
    ...
    <div class="wrap">
      <div class="foot-top">
        <div>
          ...
          <p class="foot-tag">TypeScript command center for booking disruptions, recovery queues, guest communication posture, and launch-safe hospitality operations — operator surface in the Kinetic Gain Suite. Static demo data; no live calls.</p>
        </div>
        <div class="foot-cols">
          <div class="foot-col">
            <h4>Estate</h4>
          ...
          <div class="foot-col">
            <h4>This surface</h4>
            <a href="https://github.com/mizcausevic-dev/booking-disruption-command-center">GitHub repo</a>
          </div>
        </div>
      </div>
      <div class="foot-bot">
...
function routePills(active: string, links: NavLink[]) {
  return links.map((link) => `<a class="${link.href === active ? "active" : ""}" href="${link.href}">${link.label}</a>`).join("");
}
...
function renderHero(active: string) {
  ...
  const urgentIncident = bookingLane().find((item) => item.risk === "red") ?? bookingLane()[0];
  const recoveryRisk = recoveryRisks().find((item) => item.readiness === "red") ?? recoveryRisks()[0];
  const packet = guestCommunication().find((item) => item.status !== "green") ?? guestCommunication()[0];
  return `${topbar(active)}
  <section class="hero reveal">
    <div class="acard hero-panel">
      <div class="hero-grid">
        <div class="hero-copy">
          <span class="eyebrow">Booking recovery</span>
          <div class="hero-meta">
            <span class="meta-chip">Guest trust</span>
            <span class="meta-chip">Inventory + payments</span>
            <span class="meta-chip">Operator escalation</span>
          </div>
          <h1>Where do booking disruptions quietly become guest-trust and revenue risk?</h1>
          <p class="hero-subtle">Booking Disruption Command Center keeps payment retries, OTA inventory overrides, arrival messaging, and guest-safe recovery posture visible before the next arrival wave compounds service pressure.</p>
          <div class="hero-nav">${routePills(active, links)}</div>
          <div class="hero-kpis">
            <div class="stat"><label>Incidents</label><strong>${metrics.incidents}</strong><span>Active disruption records tied to property, owner, and next move.</span></div>
            <div class="stat"><label>Urgent incidents</label><strong>${metrics.urgentIncidents}</strong><span>Red incidents where guest trust or revenue loss is already building.</span></div>
            <div class="stat"><label>Blocked recoveries</label><strong>${metrics.blockedRecoveries}</strong><span>Proof gaps still waiting on payment, inventory, or escalation evidence.</span></div>
            <div class="stat"><label>Fragile guest packets</label><strong>${metrics.fragileGuestPackets}</strong><span>Guest-facing messages still too weak to send safely.</span></div>
          </div>
        </div>
        <aside class="hero-aside">
          <div class="acard">
            <span class="metric-chip">Current watch item</span>
            <h3>${urgentIncident.property} · ${urgentIncident.channel}</h3>
            <p><strong>${urgentIncident.guestSegment}</strong><br />${urgentIncident.excerpt}</p>
          </div>
          <div class="acard">
          ...
          <div class="acard">
            ...
            <p>Payment fallback, OTA inventory, arrival messaging, vendor escalation, and guest-safe packet posture stay in one readable view instead of drifting across PMS, support, and operations.</p>
          </div>
        </aside>
      </div>
    </div>
...
export function renderOverview() {
  const metrics = summary();
  const incidents = bookingLane();
  const risks = recoveryRisks();
  const packets = guestCommunication();
  return layout(
    "Booking Disruption Command Center",
    heroShell(
      "/",
      `<section class="section">
        <div class="sh"><h2>Booking lane</h2><div class="note">Payment, inventory, and guest-message pressure that leadership can actually act on</div></div>
        <div class="section-cards">
          ${incidents.map((item) => `<article class="acard surface-card"><span class="tag ${item.risk}">${item.risk}</span><h3>${item.property} · ${item.channel}</h3><div class="surface-meta"><p><strong>${item.incidentId}</strong> · ${item.guestSegment}</p><p><strong>Owner:</strong> ${item.owner}</p></div><p class="surface-copy">${item.excerpt}</p><p class="surface-next"><strong>Next move:</strong> ${item.nextAction}</p></article>`).join("")}
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Recovery pressure</h2><div class="note">Where payment, OTA, and messaging gaps can still break guest trust</div></div>
      ...
      <section class="section">
        <div class="sh"><h2>Guest communication packets</h2><div class="note">What can safely go out next</div></div>
        <div class="packet-grid">
          ${packets.map((packet) => `<article class="acard packet-card"><div class="packet-top"><div class="packet-score">${packet.confidenceScore}%</div><span class="tag ${packet.status}">${packet.status}</span></div><h3>${packet.audience}</h3><ul class="packet-list"><li><strong>Packet:</strong> ${packet.packetId}</li><li><strong>Blocker:</strong> ${packet.blocker}</li><li><strong>Window:</strong> ${packet.replyWindowMinutes} minutes</li></ul><p class="packet-note">${packet.decisionNote}</p></article>`).join("")}
        </div>
      </section>`
    )
  );
...
export function renderBookingLane() {
  const rows = bookingLane();
  return layout(
    "Booking Disruption Command Center — Booking Lane",
    heroShell(
      "/booking-lane",
      `<section class="section">
        <div class="sh"><h2>Booking lane</h2><div class="note">Disruptions under pressure</div></div>
        <table class="ttbl">
          <thead><tr><th>Incident</th><th>Owner</th><th>Risk</th><th>Next move</th></tr></thead>
          <tbody>
            ${rows
              .map(
                (item) => `<tr>
                  <td><b>${item.property} · ${item.channel}</b><br />${item.incidentId} · ${item.guestSegment}<br />${item.excerpt}</td>
                  <td>${item.owner}</td>
...
export function renderRecoveryRisks() {
  const risks = recoveryRisks();
  return layout(
    "Booking Disruption Command Center — Recovery Risks",
    heroShell(
      "/recovery-risks",
      `<section class="section">
        <div class="sh"><h2>Recovery risks</h2><div class="note">Evidence gaps before guest promises</div></div>
        <div class="ledger">
          <div class="led">
...
export function renderGuestCommunication() {
  const packets = guestCommunication();
  return layout(
    "Booking Disruption Command Center — Guest Communication",
    heroShell(
      "/guest-communication",
      `<section class="section">
        <div class="sh"><h2>Guest communication</h2><div class="note">Reply windows and trust posture</div></div>
        <div class="board">
          ${packets
            .map(
              (packet) => `<article class="pcard">
                ...
                <ul class="check">
                  ...
                  <li>Reply window: ${packet.replyWindowMinutes} minutes</li>
                </ul>
...
export function renderVerification() {
  ...
  return layout(
    "Booking Disruption Command Center — Verification",
    heroShell(
      "/verification",
      `<section class="section">
        ...
        <div class="ledger">
          ...
          <div class="led">
            ...
            <ul>
              <li><span class="pip">→</span><div>Booking, payment, and recovery data can surface in-product without exposing unsafe write paths.</div></li>
              <li><span class="pip">→</span><div>Guest communication posture and evidence stay connected to buyer-facing dashboards.</div></li>
              <li><span class="pip">→</span><div>Synthetic data only; this repo demonstrates system shape, not production guest records.</div></li>
            </ul>
          </div>
        </div>
      </section>`
...
export function renderDocs() {
  return layout(
    "Booking Disruption Command Center — Docs",
    heroShell(
      "/docs",
      `<section class="section">
        <div class="sh"><h2>System docs</h2><div class="note">Routes and control surfaces</div></div>
        <div class="stack">
          <article class="src"><div class="src-name">Control plane</div><div class="src-tit">Booking lane</div><p>Disruption-by-disruption queue with owner routing, guest segment, and next action.</p></article>
          <article class="src"><div class="src-name">Risk map</div><div class="src-tit">Recovery risks</div><p>Required evidence, owner lanes, and impact areas for blocked service restoration.</p></article>
          <article class="src"><div class="src-name">Guest view</div><div class="src-tit">Guest communication</div><p>Packet-level confidence, blocker state, and next reply-window timing.</p></article>
          <article class="src"><div class="src-name">Embedded tie-back</div><div class="src-tit">KG Embedded</div><p>Security-first in-product analytics extension for booking recovery and service workflows.</p></article>
        </div>
        <div class="quote">
          <div class="lbl">Route surface</div>
          <div class="q"><code>/</code> · <code>/booking-lane</code> · <code>/recovery-risks</code> · <code>/guest-communication</code> · <code>/verification</code> · <code>/docs</code></div>
